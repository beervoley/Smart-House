...
public class CompileMisc implements Inlineable {
    ...
    public static Expression validateApplyFormat(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        if (args.length > 0) {
            ...
            if (f == Boolean.FALSE || ftype.isSubtype(LangObjType.stringType)) {
                ...
                ae = new ApplyExp(typeFormat.getDeclaredMethod("formatToString", (int) CONVERT), xargs);
                ...
                ae = new ApplyExp(typeFormat.getDeclaredMethod("formatToWriter", (int) NOT), args);
    ...
    public static void compileCallCC(ApplyExp exp, Compilation comp, Target target, Procedure proc) {
        ...
        if (!param.isSimple() || param.getCanRead() || param.getCanWrite()) {
            ...
            if (target instanceof ConsumerTarget) {
                comp.loadCallContext();
                code.emitInvokeStatic(typeContinuation.getDeclaredMethod("handleException$X", (int) NOT));
            } else {
                code.emitInvokeStatic(typeContinuation.getDeclaredMethod("handleException", (int) CONVERT));
    ...
    public static Expression validateApplyMap(ApplyExp exp, InlineCalls visitor, Type required, Procedure xproc) {
        ...
        if (collect) {
            Expression[] consArgs = new Expression[CONVERT];
            consArgs[0] = doit;
        ...
        while (true) {
            ...
            Expression[] compArgs = new Expression[CONVERT];
            compArgs[0] = new ReferenceExp(largs[i2]);
            compArgs[1] = empty;
            letExp.body = new IfExp(visitor.visitApplyOnly(new ApplyExp(mproc.isEq, compArgs), null), collect ? new ReferenceExp(resultDecl) : QuoteExp.voidExp, letExp.body);
